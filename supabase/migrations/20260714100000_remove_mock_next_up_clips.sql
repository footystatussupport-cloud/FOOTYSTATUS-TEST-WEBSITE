-- Remove temporary/demo Next Up clips and make the feed real-content-only.
-- The feed should only return approved clips connected to an actual
-- player_profiles row. Legacy sample rows from the early clips seed are
-- deleted here and will stay gone after rebuilds/deployments because this
-- migration runs after the original seed migration.

with mock_clip_ids as (
  select c.id
  from public.clips c
  where c.video_url = 'https://example.com/clip.mp4'
     or c.thumbnail_url like 'https://images.unsplash.com/photo-1560272564-c83b66b1ad12%'
     or lower(coalesce(c.description, '')) = 'check out my latest highlights!'
     or lower(coalesce(c.title, '')) in (
       'amazing goal vs fc dallas',
       'hat-trick highlights',
       'best assists compilation',
       'defensive skills reel',
       'speed & dribbling',
       'training session highlights'
     )
)
delete from public.clips c
using mock_clip_ids m
where c.id = m.id;
...
begin
  ...
  with eligible as (
    ...
    join public.player_profiles pp
      on pp.id = c.player_id
      or pp.user_id = c.user_id
    ...
    where c.review_status = 'approved'
      ...
      and (
        ...
        or pp.player_gender = v_effective_gender
      )
      and not exists (
        select 1
